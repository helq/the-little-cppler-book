...
Notice how we wrote FOUR (4) `if` statements but only 3 numbers appear. Why?

(answer, well, we start with `i==0`, then we add `1` and print, and we do it again, and
again, and again, and every time we ask if `i` is smaller than 3, and guess what, it isn't
always smaller than 3, not forever)

---

But what if we wanted to use a different limit, not 3 but 18, then we would need at least
20 if statements just like the one above and that's kinda stupid.

Introducing... `while`{.cpp}, it works like many `if` one after the other with no end, it
stops when the question returns `false`. Let's see an example:
